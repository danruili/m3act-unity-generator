...
public class activity_1_agent : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        */
        ...
        {
            if (distance(model.transform.position, this.transform.position) != 0f && distance(model.transform.position, this.transform.position) <= 0.3f)
            {

                Vector3 from = model.transform.position - this.transform.position;
                if (Vector3.Angle(from, to) < 20)
                {
    ...
    /*private void OnTriggerEnter(Collider other)
    {
        ...
        {
            Vector3 to = this.transform.forward;
            Vector3 from = other.transform.position - this.transform.position;
