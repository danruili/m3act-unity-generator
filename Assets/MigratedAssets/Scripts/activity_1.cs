...
public class activity_1 : MonoBehaviour
{
    ...
    List<GameObject> model_list;
    List<GameObject> other_models;
    ...
    void Init()
    {
        ...
        model_list = new List<GameObject>();
        other_models = new List<GameObject>();
        ...
        //GameObject SyntheticHumans = GameObject.Find("HumanPool__HumanGenerationRandomizer");
        //Debug.Log(SyntheticHumans);
        //Debug.Log(SyntheticHumans.transform.childCount);
    ...
    private Vector3[] placeInRect()
    {
        ...
        for (; i < numEachSide; i++)
        {
            ...
            character.SetActive(true);
            model_list.Add(character);
            positions[i] = new Vector3(xpos + Random.Range(-0.25f, 0.25f), -0.9277931f, zpos + Random.Range(-0.25f, 0.25f));
            xpos = xpos + dist;
        }
        xpos = xorigin;
    ...
    private void agentsActive()
    {
        ...
        for (int i = 0; i < numOfAgents; i++)
        {
            ...
            model_list[i].transform.position = positions[i];
    ...
    private void agentsSetting()
    {
        ...
        for (int i = 0; i < model_list.Count; i++)
        {
            ...
            model_list[i].GetComponent<Animator>().speed = Random.Range(1.5f, 2.2f);
            ...
            */
            ...
            MixamoBlendTree t = AnimationManager.GetGroupByName("Walking").RandomBlendTree();
    ...
    private void otherAgentsActive()
    {
        ...
        for (int i = 0; i < numOfOtherAgents; i++)
        {
            ...
            otheragents[i] = index;
            existedIndex.Add(index);
            float xPos = Random.Range(-10, 10)+origin.x;
            float zPos = Random.Range(-10, 10)+origin.z;
        ...
        for (int i = 0; i < numOfOtherAgents; i++)
        {
            ...
            models[otheragents[i]].GetComponent<NavMeshAgent>().enabled = false;
    ...
    private void otherAgentsSetting()
    {
        for (int i = 0; i < numOfOtherAgents; i++)
        {
            ...
            other_models[i].GetComponent<Animator>().speed = Random.Range(1.5f, 5.2f);
    ...
    {        
        ...
        for (int i = 0; i < model_list.Count; i++)
        {
            ...
            foreach (GameObject model_ in model_list)
            {
                ...
                if (dist != 0f && dist <= .8f && Vector3.Angle(from, to) < 60)
                {
                    model.GetComponent<Animator>().speed = Mathf.Min(model.GetComponent<Animator>().speed * 0.96f, 0.10f);
                }
                else
                {
